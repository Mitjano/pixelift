...
export default function VectorizePage() {
  ...
  const t = useTranslations('vectorizePage');
  const tTools = useTranslations('tools.vectorize');
  const locale = useLocale();

  return (
    <ToolsLayout>
      <BreadcrumbSchema items={getToolBreadcrumbs(tTools('name'), 'vectorize', locale)} />
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="absolute top-0 left-1/4 w-96 h-96 bg-teal-300/30 dark:bg-teal-600/20 rounded-full blur-3xl" />
        <div className="absolute top-20 right-1/4 w-80 h-80 bg-cyan-300/20 dark:bg-cyan-600/20 rounded-full blur-3xl" />
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-full h-px bg-gradient-to-r from-transparent via-teal-500/50 to-transparent" />
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-16 md:py-20">
          <div className="text-center max-w-4xl mx-auto">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-teal-100 dark:bg-teal-600/20 border border-teal-300 dark:border-teal-500/30 rounded-full text-sm mb-6">
              ...
              <span className="text-teal-600 dark:text-teal-300">{t('badge')}</span>
            </div>

            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 leading-tight">
              <span className="text-gray-900 dark:text-white">{t('title')} </span>
              <span className="bg-gradient-to-r from-teal-500 via-cyan-500 to-teal-500 dark:from-teal-400 dark:via-cyan-400 dark:to-teal-400 bg-clip-text text-transparent">
                {t('titleHighlight')}
              </span>
            </h1>
      ...
      <section className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {[
            { key: 'fullColor', gradient: 'from-teal-100 dark:from-teal-500/20 to-cyan-100 dark:to-cyan-500/20' },
            { key: 'grayscale', gradient: 'from-gray-100 dark:from-gray-500/20 to-slate-100 dark:to-slate-500/20' },
            { key: 'blackWhite', gradient: 'from-slate-100 dark:from-slate-500/20 to-gray-100 dark:to-gray-500/20' },
          ].map((option) => (
            <div
              key={option.key}
          ...
          <div className="grid md:grid-cols-4 gap-6">
            {['step1', 'step2', 'step3', 'step4'].map((step, idx) => (
              <div key={step} className="text-center">
                <div className="w-12 h-12 bg-gradient-to-br from-teal-500 to-cyan-500 rounded-full flex items-center justify-center text-white font-bold text-xl mx-auto mb-4">
                  {idx + 1}
                </div>
      ...
      {/* Technology Section */}
      <section className="max-w-7xl mx-auto px-6 py-12">
        <div className="bg-gradient-to-br from-teal-100 to-cyan-100 dark:from-teal-900/30 dark:to-cyan-900/30 rounded-2xl border border-teal-300 dark:border-teal-700/50 p-8">
      ...
      {/* Related Tools */}
      <section className="max-w-7xl mx-auto px-6 py-12">
        <RelatedTools currentSlug="vectorize" />
      </section>

      {/* CTA Section */}
      <section className="max-w-7xl mx-auto px-6 py-16">
        <div className="bg-gradient-to-r from-teal-100 to-cyan-100 dark:from-teal-900/50 dark:to-cyan-900/50 border border-teal-300 dark:border-teal-500/30 rounded-2xl p-8 md:p-12 text-center">
          <h2 className="text-3xl md:text-4xl font-bold mb-4 text-gray-900 dark:text-white">
            {t('cta.title')}
          </h2>
          ...
          {!session ? (
            <Link
              href="/auth/signin"
              className="inline-block px-8 py-4 bg-gradient-to-r from-teal-600 to-cyan-600 hover:from-teal-700 hover:to-cyan-700 text-white rounded-xl font-semibold text-lg transition shadow-lg shadow-teal-500/25"
            >
              {t('cta.getStarted')}
            </Link>
          ) : (
            <button
              ...
              className="px-8 py-4 bg-gradient-to-r from-teal-600 to-cyan-600 hover:from-teal-700 hover:to-cyan-700 text-white rounded-xl font-semibold text-lg transition shadow-lg shadow-teal-500/25"
            >
              {t('cta.startAction')}
            </button>
          )}
